...
describe('image adapters', () => {
  ...
  it('builds Gemini generateContent image request with inline reference parts', async () => {
    mockedAxios.post.mockResolvedValue({ data: { responseId: 'g-1', candidates: [{ content: { parts: [{ inlineData: { data: 'abc', mimeType: 'image/png' } }] } }] } });
    const result = await generateWithGemini({ apiKey: 'key', baseUrl: 'https://api.example.com' }, { model: 'gemini-3-pro-image-preview', prompt: 'cat', images: [{ data: 'ref', mimeType: 'image/jpeg' }], size: '16:9', resolution: '2K', count: 1 });
    expect(mockedAxios.post).toHaveBeenCalledWith(expect.stringContaining('/v1beta/models/gemini-3-pro-image-preview:generateContent'), expect.objectContaining({ generationConfig: expect.objectContaining({ responseModalities: ['IMAGE'], imageConfig: { aspectRatio: '16:9', imageSize: '2K' } }) }), expect.objectContaining({ headers: expect.objectContaining({ 'x-goog-api-key': 'key' }) }));
  ...
  it('parses supported image response forms', () => {
    ...
    expect(parseImageResponse({ candidates: [{ content: { parts: [{ inlineData: { data: 'i', mimeType: 'image/webp' } }] } }] }).images[0].mimeType).toBe('image/webp');
  });
});
