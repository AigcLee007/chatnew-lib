...
export function parseImageResponse(data: { id?: string; data?: OpenAIItem[]; candidates?: Array<{ content?: { parts?: Array<{ inlineData?: { data?: string; mimeType?: string }; image?: string; image_url?: string | { url?: string } }> } }> }): ImageGenerationResponse {
  ...
  (data.data ?? []).forEach((item, index) => {
    ...
    const value = item.b64_json ?? item.url ?? (typeof item.image === 'string' ? item.image : objectImage?.data ?? objectImage?.url) ?? (typeof item.image_url === 'string' ? item.image_url : item.image_url?.url);
    if (value) images.push({ data: value, mimeType: objectImage?.mimeType ?? (value.startsWith('data:image/') ? (value.match(/^data:([^;]+)/)?.[1] ?? 'image/png') : 'image/png'), index });
  });
  (data.candidates ?? []).flatMap((candidate) => candidate.content?.parts ?? []).forEach((part) => {
    const inline = part.inlineData;
    const value = inline?.data ?? part.image ?? (typeof part.image_url === 'string' ? part.image_url : part.image_url?.url);
    if (value) images.push({ data: value, mimeType: inline?.mimeType ?? 'image/png', index: images.length });
  });
...
export async function generateWithOpenAI(config: ImageAdapterConfig, request: ImageGenerationRequest): Promise<ImageGenerationResponse> {
