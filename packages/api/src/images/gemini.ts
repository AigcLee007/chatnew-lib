...
function parseGeminiResponse(data: {
  ...
  const images: ImageGenerationResult[] = (data.candidates ?? [])
    .flatMap((candidate) => candidate.content?.parts ?? [])
    .flatMap((part, index) =>
      part.inlineData
        ? [{ data: part.inlineData.data, mimeType: part.inlineData.mimeType || 'image/png', index }]
        : [],
    );
  return {
    images,
...
export async function generateWithGemini(
  ...
  const parts: GeminiPart[] = [
    ...
    ...(request.images ?? []).map((image) => ({
      inlineData: { data: image.data.replace(/^data:[^,]+,/, ''), mimeType: image.mimeType },
    })),
  ];
  const response = await axios.post(
    `${config.baseUrl.replace(/\/$/, '')}/v1beta/models/${request.model}:generateContent`,
    {
      contents: [{ role: 'user', parts }],
      generationConfig: {
        responseModalities: ['IMAGE'],
        imageConfig: { aspectRatio: request.size, imageSize: request.resolution },
      },
    },
    {
