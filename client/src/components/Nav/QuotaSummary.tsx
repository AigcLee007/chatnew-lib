...
export default function QuotaSummary() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setQuota(response.ok ? await response.json() : null);
    } finally {
      setLoading(false);
    }
  };
  return (
    <Menu.MenuProvider open={open} setOpen={(value) => { setOpen(value); if (value && !quota) void refresh(); }} placement="right-start">
      ...
      <Menu.Menu portal className="account-settings-popover popover-ui z-[126] w-[260px] rounded-lg p-4">
        ...
        {quota ? <dl className="mt-3 grid grid-cols-2 gap-2 text-xs"><dt className="text-text-secondary">总额度</dt><dd>{quota.total ?? '-'}</dd><dt className="text-text-secondary">已使用</dt><dd>{quota.used ?? '-'}</dd><dt className="text-text-secondary">剩余</dt><dd>{quota.remaining ?? '-'}</dd><dt className="text-text-secondary">使用率</dt><dd>{quota.percentage == null ? '-' : `${quota.percentage}%`}</dd></dl> : <p className="mt-3 text-xs text-text-secondary">暂无额度数据</p>}
      </Menu.Menu>
    </Menu.MenuProvider>
  );
}
